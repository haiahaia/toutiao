<template>
    <div>
    <!-- Header 区域 -->
    <van-nav-bar
      fixed
      title="文章详情"
      left-arrow
      @click-left="$router.back()"
    />

    <!-- 文章--等待加载中 -->
    <!-- <van-loading color="#1989fa">
      文章加载ing...
    </van-loading> -->

    <div>
      <!-- 文章信息区域 -->
      <div class="article-container">
        <!-- 文章标题 -->
        <h1 class="art-title">{{ artObj.title}}</h1>

        <!-- 用户信息 -->
        <van-cell center :title="artObj.aut_name" :label="formatDate(artObj.pubdate)">
          <template #icon>
            <img :src="artObj.aut_photo" alt="" class="avatar" />
          </template>
          <template #default>
            <div>
              <van-button type="info" size="mini"
              @click="followedFn(true)"
              v-if="isfollow"
              >已关注</van-button>
              <van-button icon="plus" type="info" size="mini" plain
              @click="followedFn(false)"
              v-else
                >关注</van-button
              >
            </div>
          </template>
        </van-cell>

        <!-- 分割线 -->
        <van-divider></van-divider>

        <!-- 文章内容 -->
        <div class="art-content" v-html="artObj.content">
        </div>

        <!-- 分割线 -->
        <van-divider>End</van-divider>

        <!-- 点赞(文章) -->
        <!-- attitude:  -1: 无态度，0-不喜欢，1-点赞 -->
        <div class="like-box">
          <van-button icon="good-job" type="danger" size="small"
          @click="loveFn(true)"
          v-if="islike"
            >已点赞</van-button
          >
          <van-button icon="good-job-o" type="danger" plain size="small"
          @click="loveFn(false)"
          v-else
            >点赞</van-button
          >
        </div>
      </div>

      <!-- 文章评论部分 -->
      <div>
        <CommentList></CommentList>
      </div>
    </div>

  </div>
</template>

<script>
import { ArticleDetailAPI, followingsAPI, unfollowingsAPI, likeArticleAPI, removeLikeArticleAPI } from '@/api'
import { timeAgo } from '@/utils/date'
import CommentList from './CommentList'
export default {
  name: 'ArticleDetail',
  data () {
    return {
      artObj: {},
      isfollow: false,
      islike: false
    }
  },
  async created () {
    const ref = await ArticleDetailAPI({
      artId: this.$route.query.artId
    })
    console.log(ref)
    this.artObj = ref.data.data
    this.isfollow = this.artObj.is_followed
    this.islike = this.artObj.attitude
    // console.log(this.Object.content)
  },
  components: {
    CommentList
  },
  methods: {
    formatDate: timeAgo,
    followedFn (value) {
      if (value) {
        unfollowingsAPI({
          userId: this.artObj.aut_id
        })
        this.isfollow = !this.isfollow
      } else {
        followingsAPI({
          userId: this.artObj.aut_id
        })
        this.isfollow = !this.isfollow
      }
    },
    loveFn (value) {
      if (value) {
        removeLikeArticleAPI({
          artId: this.artObj.art_id
        })
        this.islike = !this.islike
      } else {
        likeArticleAPI({
          artId: this.artObj.art_id
        })
        this.islike = !this.islike
      }
    }
  }
}
</script>

<style scoped lang="less">
/deep/ a {
  all: unset
}
.article-container {
  padding: 10px;
  margin-top: 46px;
}
.art-title {
  font-size: 16px;
  font-weight: bold;
  margin: 10px 0;
}

.art-content {
  font-size: 12px;
  line-height: 24px;
  width: 100%;
  overflow-x: scroll;
  word-break: break-all;
  /deep/ img {
    width: 100%;
  }
  /deep/ pre {
    white-space: pre-wrap;
    word-wrap: break-word;
  }
}

.van-cell {
  padding: 5px 0;
  &::after {
    display: none;
  }
}

.avatar {
  width: 60px;
  height: 60px;
  border-radius: 50%;
  background-color: #f8f8f8;
  margin-right: 5px;
  border: none;
}

.like-box {
  display: flex;
  justify-content: center;
}

// 加载中居中样式
.van-loading{
  text-align: center;
  padding-top: 46px;
}
</style>
